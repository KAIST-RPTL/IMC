...
99  if (do_gmsh_vrc) close(prt_tet_vrc)
    ...
    if(icore==score) print *, time_dep_done-time_dep
end do BURNUP
...
subroutine process_MSR_prec()
    ...
    open(prt_fuel_mv, file='MSR_prec',action='read',status='old')
    do i = 1,n_act
        do j = 1,8
...
if ( icore == score ) then
    ...
    !write(*,13), '  LAMSUM',1E2*(AVG(lamarr(1:n_act-latent,0))),'+/-',1E2*(STD_M(lamarr(1:n_act-latent,0))),' scale:1E-2'
    do i = 1,8
        ...
        !write(*,15), '   LAM',i,AVG(lamarr(1:n_act-latent,i))*1E2,'+/-',STD_M(lamarr(1:n_act-latent,i))*1E2,' scale:1E-2'
    enddo
    !write(*,14), '  ALPHA',AVG(alphaarr)*1E-6,'+/-',STD_M(alphaarr)*1E-6,' scale:1E6'
    print *, AVG(alphaarr), STD_M(alphaarr)
