...
!    real(8) :: r, z, cost, inst       !> R, Z, cos(t), sin(t) [cm]
!    integer :: i, ii, j, jj
...
!
!        ! 3. vr, vz = v(i,j)
!        vr = velocity_r(i); vz = velocity_z(j)
...
!            if ( z >= active_z(nz+1) )
!                ! 7-3-1.  IF r <= riser_r !> Escaped
!                if ( r <= riser_r ) then
!                    ! 7-3-1-1.   IF t_left < t_recirc
!                    if( t_left <= t_recirc ) then
!                        ! 7-3-1-1-1.    KILL and DONE
!                        alive = .false.
!                        return
!                    ! 7-3-1-2.   ELSE
!                    else
!                        ! 7-3-1-2-1.    t_left -= t_recirc
!                        t_left = t_left - t_recirc
!                        ! 7-3-1-2-2.    j = 1, z = 0
!                        j = 1; z = 0d0
!                        ! 7-3-1-2-3.    sample r and theta randomly
!                        r = active_r(nr+1) * sqrt(rang())
!                        theta = 2d0 * pi * rang()
!                        cost = cos(theta); sint = sin(theta);
!                ! 7-3-2.  ELSE            !> TRAPPED
!                else
!                    ! 7-3-2-1.   z = active_z(nz+1)
!                    z = active_z(nz+1)
