...
!===============================================================================
    subroutine set_dynamic_bank() 
        ...
        if (icore == score) then 
            ...
            print '(a,2I,2E13.5)', 'asdfsfas', size(prompt_bank), isize , &
                sum(prompt_bank(:)%wgt)/real(size_bank-M,8), &
                sum(dynamic_bank(1:M)%wgt)/real(M,8)
    ...
    subroutine collision_dynamic_MG(p)
        ...
        if (rang() < sum(XS_MG(imat)%sig_scat(p%g,:)/temp)) then ! scattering
            ...
            do i_group = 1, size(XS_MG(p%material)%sig_scat(p%g,:))
                if (rnum < sum(XS_MG(p%material)%sig_scat(p%g,1:i_group)) &
                    /sum(XS_MG(p%material)%sig_scat(p%g,:))) then 
                    idx_group = i_group
                    exit
                endif 
            enddo 
            ...
            do i_group = 1, size(XS_MG(p%material)%chi(:))
                if (rnum < sum(XS_MG(p%material)%chi(1:i_group)) &
                    /sum(XS_MG(p%material)%chi(:))) then 
                    idx_group = i_group
                    exit
                endif 
            enddo
