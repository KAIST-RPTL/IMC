...
3        continue
    ...
    function lattice_coord (this, xyz0) result(coord_lat)
        ...
        !if(coord_lat(1)==14 .and. coord_lat(2) == 13) &
        !    write(*,'(f8.3,f8.3,I,I)') (xyz0(1:2) - this%xyz(1:2))/this%pitch(1:2), coord_lat(1:2)
    end function 
    ...
    subroutine lat_distance (this,surflist, xyz, uvw,i_xyz, d_surf, idx_surf)
        ...
        elseif(this%lat_type==2) then
            ...
            if(abs(xyz_(1)+xyz_(2)*tmp*2.d0) > this%pitch(1)) d_surf = INFINITY
            if(abs(xyz_(1)-xyz_(2)*tmp*2.d0) > this%pitch(1)) d_surf = INFINITY
        ...
        elseif (this%lat_type == 2) then 
            ...
            ! -\/     \/
            tmp = sqrt(3.d0)*0.5d0
            newu = tmp*uvw(2)-0.5d0*uvw(1)
            
            dxyz(1) = int(sign(1.0d0,uvw(1)))
            dxyz(2) = int(sign(1.0d0,newu))
            ...
            !do ix = 1,this%n_xyz(1)
                ...
                xyz_(3) = xyz(3) + this%pitch(3)*(i_xyz(3)-iz)
                !print *, 'TST', xyz_(1:2)-xyz(1:2), (i_xyz(1)-ix), (i_xyz(2)-iy)
                j = this%lat(ix,iy,iz)
                if(allocated(universes(j)%r)) then
                associate(c=>cells(universes(j)%cell(universes(j)%ncell)))
                if(cell_contains_xyz(c,xyz_)) then
                ...
                if(cell_contains_xyz(c,xyz_)) then
                    ...
                    if (d_surf>d_temp) then
                        d_surf = d_temp
                        !print *, 'Case2', xyz(1:2) + uvw(1:2) * d_surf, c%cell_id
                        idx_surf = idx_temp
                    endif
                endif
                end associate
            ...
            enddo
            enddo
            !print *, 'Z', xyz(1:2) + uvw(1:2) * d_surf
		elseif (this%lat_type == 3) then 
            ! TWO DIRECTIONS
            tmp = sqrt(3.d0)*0.5d0
            newv = tmp*uvw(1)-0.5d0*uvw(2)
            dxyz(1) = int(sign(1.d0,newv))
            dxyz(2) = int(sign(1.d0,uvw(2)))
