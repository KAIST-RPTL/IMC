...
400             close(isom_brn)
            ...
            do imat = 1, n_materials
                ...
                !samarium = 0.d0
                mat => materials(imat)
                if(icore==score) print *, 'MATDEP', imat
                if (icore==score) then 
                    write(prt_bumat, *) '' 
                ...
                DO_ISO: do mt_iso = 1,num_iso
                    ...
                    if(jnuc==0) cycle
                    do i = 1,4
                    !if(icore==score) print *, imat, ace(iso)%zaid, mat%ogxs(mt_iso,i)*real_flux
                    enddo
        ...
        if(icore==score) print *, 'CRAM solved for mat:',imat 
        ...
        !!! Sort with NUMDEN * sumval(mat%ogxs(iso,1:7)) !!! 
        allocate(mat%OPTNUMDEN(1:mat%n_iso)); mat%OPTNUMDEN = 0
        mat%OPTNUMDEN(1) = 1
        do mt_iso = 2, mat%n_iso
            iso = iso_idx(mt_iso)
            ace_iso = find_ACE_iso_idx_zaid(zai_idx(iso))
            do pos = 1,mt_iso-1
                tmp = mat%OPTNUMDEN(pos)
                ace_tmp = find_ACE_iso_idx_zaid(zai_idx(iso_idx(tmp)))
                if(sum(mat%ogxs(ace_iso,1:7))*mat%full_numden(iso) >= sum(mat%ogxs(ace_tmp,1:7))*mat%full_numden(iso_idx(tmp))) exit
            enddo
            !if(icore==score) print *, imat, ace(ace_iso)%zaid, ace(ace_tmp)%zaid, pos, mat%full_numden(iso)*sum(mat%ogxs(ace_iso,1:7)), mat%full_numden(iso_idx(tmp))*sum(mat%ogxs(ace_tmp,1:7))
            mat%OPTNUMDEN(pos+1:mt_iso) = mat%OPTNUMDEN(pos:mt_iso-1)
            mat%OPTNUMDEN(pos) = mt_iso
        enddo

        !do mt_iso = 1,mat%n_iso
        !    if(icore==score) print *, imat, zai_idx(iso_idx(mat%OPTNUMDEN(mt_iso)))
        !enddo

        do pos = 1,mat%n_iso
            mt_iso = mat%OPTNUMDEN(pos)
        ...
        !    ! find ace_idx
            ...
            if (tmp /= 0 ) then 
                i = i + 1
                mat%ace_idx(pos) = tmp
                mat%numden(pos)  = mat%full_numden(iso_idx(mt_iso))
                ...
                if (icore==score) then 
                    ...
                    ! ADENS
                    write(prt_bumat, '(a15,e14.5,e14.5,e14.5)') ace(mat%ace_idx(pos))%library, mat%numden(pos)*barn, mat%numden(pos)*barn*N_AVOGADRO*sum(mat%ogxs(mat%ace_idx(pos),1:7)), mat%numden(pos)*barn*N_AVOGADRO*mat%ogxs(mat%ace_idx(pos),7)
                    ! A
    ...
    subroutine Init_burnup 
        ...
        if(icore==score) print *, '   BURNUP INITIALIZED'
        !> Xe equilibrium
        if (.not. do_Xe_search) return 
        Xe_prod(:) = 0.0d0
        Xe_trans(:) = 0.0d0
    ...
    subroutine setmat
        ...
        do imat = 1, n_materials
            ...
            do mt_iso = 1, materials(imat)%n_iso
                ...
                nnum = mnum - anum
                if(nnum<=0) then
                    if(materials(imat)%depletable==.true. .and. do_burn .and. nuclide(0,0,anum)%idx>0) then
                        materials(imat)%full_numden(nuclide(0,0,anum)%idx) = materials(imat)%numden(mt_iso)
                    endif
                else
