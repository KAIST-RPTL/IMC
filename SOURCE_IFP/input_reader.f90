...
subroutine read_geom
    ...
    if(file_exists==.false.) then
        if (icore==score) print *, "INPUT ERROR :: NO geom_mat.inp file"
        stop 
    end if 
...
subroutine READ_CTRL        
    ...
    !if(icore==score) print *, 'MAKETEST1'
    file_exists = .false.
    inquire(file=trim(filename),exist=file_exists)
    if(file_exists==.false.) then
        ...
        case('A') 
            Read_Card_A : do
				...
				case("FUEL_SPEED_AXIAL")
                    ...
                    if(Equal/="=") call Card_Error(Card_Type,Char_Temp)
                    if(fuel_speed>=0.d0) do_fuel_mv = .true.
                ...
                if (Char_Temp(1:3)=="MAT" .or. Compare_String(Char_Temp(1:3),"mat")) then
                    ...
                    Read_Mat : do 
						...
						case("MAT_TYPE")
                            ...
                            case("VOID"); CE_mat_ptr%mat_type = 4
							end select                        
