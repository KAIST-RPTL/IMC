module geometry
            ...
            associate(c => cells(i_cell))
                CELL_TYPE: if (c % filltype == FILL_MATERIAL) then
                    ...
                    if ( p%material == 0 ) then 
                        p%alive = .false. 
                        print *, 'MAT0', j, i_cell, p%coord(1)%xyz(1:3)
					else 
						p%alive = .true. 
                    endif
                ...
                elseif (c % filltype == FILL_LATTICE) then CELL_TYPE
                    ...
                    associate (latptr => lattices(find_lat_idx(lattices, c % fill)))
                        ...
                        !print *, 'LATUNIV', p % coord(j+1) % universe, i_xyz(1:2)
    ...
    subroutine distance_to_boundary(p, dist, surface_crossed)!, level)!, lattice_translation, next_level)
        ...
        integer :: idx
        integer :: i, j, j_idx, k !!! ELIM. k after testing
        ...
        LEVEL_LOOP: do 
            ...
            idx = p % coord(j) % cell
!            if(idx == 0) then
!                print *, 'TST', j, p % coord(j) % xyz(1:2), p%n_coord
!                do k = 1,p%n_coord
!                    print *, 'PCOORD', k, p%coord(k)%universe, p % coord(k) % cell
!                enddo
!                call find_cell(p, found,tmpcell)
!                !print *, 'REFINE', found,  p % coord(j)%cell, tmpcell
!            endif
            ! =======================================================================
...
!===============================================================================
    ...
    subroutine cross_surface(p, surface_crossed)
        ...
        if (surfaces(surface_crossed)%bc == 1) then     !> Vacuum BC
            ...
            !write(*,'(A4,I1.1,F10.3,F10.3,F10.3)') 'CRSS', surface_crossed, p % coord(1) % xyz(:)
            p % alive = .false.
			return 
        ...
        elseif (surfaces(surface_crossed)%bc == 2) then !> Reflective BC 
            ...
            p % coord(1) % xyz(:) = p % coord(1) % xyz(:) + 100*TINY_BIT * p % coord(1) % uvw(1:3)
            !write(*,'(A4,I1.1,F10.3,F10.3,F10.3)') 'CRSS', surface_crossed, p % coord(1) % xyz(:)
            ...
            ! TESTING...
            !print *, 'REFL', p % coord(1) % xyz(1:3), p % coord(1) % uvw(1:3)
    ...
    subroutine reflective_bc (uvw, xyz, surface_crossed)
        ...
        !print *, 'REF', uvw, xyz, surface_crossed
        ...
        case (11) !> hexxc
            ...
            if ((xyz_(1) >= r-1.1*TINY_BIT).and.(xyz_(1) <= r+1.1*TINY_BIT))then
                uvw(1) = -uvw(1)
            elseif((xyz_(2)>=r-1.1*TINY_BIT).and.(xyz_(2)<=r+1.1*TINY_BIT)) then
                ...
                uvw(1) = uvw(1)+0.5d0*a
                uvw(2) = uvw(2)-tmp*a
            elseif((xyz_(3)>=r-1.1*TINY_BIT).and.(xyz_(3)<=r+1.1*TINY_BIT)) then
                ! REFLECTION to <tmp,-0.5> -> n = <0.5,tmp>
        ...
        case(12) !> hexyc
            ...
            r = surfaces(surface_crossed)%parmtrs(3)
            !print *, 'STUCK?', xyz(1:2)
            if ((xyz_(1) >= r-100*TINY_BIT).and.(xyz_(1) <= r+100*TINY_BIT))then
                ! REFLECTION to <tmp,-0.5>
                a = uvw(1)*tmp*2.d0-uvw(2)
                uvw(1) = uvw(1)-tmp*a
                uvw(2) = uvw(2)+0.5d0*a
            elseif((xyz_(2)>=r-100*TINY_BIT).and.(xyz_(2)<=r+100*TINY_BIT)) then
                ! REFLECTION to <tmp,0.5>
                a = uvw(1)*tmp*2.d0+uvw(2)
                uvw(1) = uvw(1)-tmp*a
                uvw(2) = uvw(2)-0.5d0*a
            elseif((xyz_(3)>=r-100*TINY_BIT).and.(xyz_(3)<=r+100*TINY_BIT)) then
                uvw(2) = -uvw(2)
            else ! EXCEPTION MSG
                print *, 'particle not on surface'
                print *, 'ixyz', xyz_(1)-r,xyz_(2)-r,xyz_(3)-r
    ...
    recursive subroutine find_cell_xyz(xyz, idx_univ, cell_idx)
        ...
        CELL_LOOP: do i = 1, n
            ...
            if(cell_xyz(cells(i_cell), xyz)) then
                found = .true.
                !print *, 'XYZ', universes(idx_univ)%univ_id, xyz
                exit
            endif
            !stop
        end do CELL_LOOP
            ...
            associate(c => cells(i_cell))
                ...
                elseif (c % fill_type() == FILL_LATTICE) then CELL_TYPE
                    ...
                    associate (latptr => lattices(find_lat_idx(lattices,c % fill)))
                        ...
                        !print *, 'lattice coordinate :', i_xyz(1:2)
                        ! Store lower level coordinates
                        !print *, xyz(1:2), i_xyz(1)*25 + i_xyz(2) -25
                        tmp = xyz
