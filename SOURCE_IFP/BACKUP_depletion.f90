    ...
    subroutine getENDFdepletionlibrary
        ...
        allocate(nuclide(inum,nnum,anum)%yield_E(n_y_eg))
                do eg = 1,n_y_eg
                    ...
                    enddo
                enddo
                nuclide(0,fssn_nnum,fssn_anum)%yield_E = Ep*1.d-6
                do eg = 1,n_y_eg
...
400             close(isom_brn)
            ...
            do i = 1,nfssn
                ...
                ratio = sum(yield_data(:,i))/2.d0
                !if(ratio>0.d0) yield_data(:,i) = yield_data(:,i)/ratio
                !if(icore==score) print *, fssn_zai(i), ratio
            enddo
            ...
            do jnuc = 1, nnuc
                ...
                elseif(prod(k,1)==0 .and. prod(k,2)==0 .and. prod(k,3)==0) then
                    cycle
            ...
            do imat = 1, n_materials
                ...
                DO_ISO: do mt_iso = 1,num_iso
                    ...
                    ! 21/10/18 MODIFIED TO CONSIDER ISOMERIC BRANCH
                    zai = ace(iso)%zaid*10+inum
                    ism = 0
!                    if(ace(iso)%zaid==62149) samabs = mat%ogxs(mt_iso,1)
                    do i = 1,num_brn
                    ...
                    if(ism > 0) then
                        ...
                        !TESTING
                        !if(icore==score) print *, ace(iso)%zaid, ism, gnd_frac(ism)
                    else
                        knuc = nuclide(0,nnum+1,anum)%idx
            ...
            if(ace(iso)%jxs(21)/=0 .or. allocated(ace(iso)%sigf)) then
                ...
                if(nuclide(inum,nnum,anum)%fy_idx>0) then   
                    ...
                    do j=1,nfp
                        ...
                        if(knuc/=0) bMat(knuc,jnuc) = &
                            bMat(knuc,jnuc) + real_flux*mat%ogxs(mt_iso,4)*yield_data(j,nuclide(inum,nnum,anum)%fy_idx)*bstep_size
                        if(anum1==61 .and. mnum1==149 .and. icore==score) then
                            !print *, 'Nd149', anum, mnum, fssn_zai(fy_midx), yield_data(j,fy_midx)*mat%ogxs(mt_iso,4)*barn*mat%full_numden(jnuc)*real_flux/nuclide(0,89,60)%lambda
                        endif
                    end do
                else
                if(anum>=89) then !For actinides
                ...
                ! Use U-235 otherwise
                !if(icore==score) print *, 'FY TEST', fy_midx, anum, nnum+anum,fssn_zai(fy_midx) 
                do j=1,nfp
                    anum1 = fp_zai(j)/10000
                    mnum1 = (fp_zai(j) - anum1*10000)/10
                    nnum1 = mnum1 - anum1
                    ...
                    knuc = nuclide(inum1,nnum1,anum1)%idx
                    if(knuc/=0) bMat(knuc,jnuc) = &
                        bMat(knuc,jnuc) + real_flux*mat%ogxs(mt_iso,4)*yield_data(j,fy_midx)*bstep_size
                    if(anum1==61 .and. mnum1==149 .and. icore==score) then
                        !print *, 'Nd149', anum, mnum, fssn_zai(fy_midx), yield_data(j,fy_midx)*mat%ogxs(mt_iso,4)*mat%full_numden(jnuc)*barn*real_flux/nuclide(0,89,60)%lambda
                    endif
                end do
                end if
                end if
                ...
                !samarium = samarium + mat%ogxs(mt_iso,4)*yield_data(pm149,fy_midx)*mat%full_numden(jnuc)*barn
                !if(icore==score) print *, ace(iso)%zaid, pm149, yield_data(pm149,fy_midx)
                ! Continuous refueling
                if(refuel) then
                do i = 1,n_rf
                    knuc = nuclide(0,mnum_rf(i)-anum_rf(i),anum_rf(i))%idx
                    if(knuc>0) bMat(knuc,jnuc) = bMat(knuc,jnuc) + &
                    real_flux*mat%ogxs(mt_iso,4)*r_rf(i)*bstep_size
                enddo
                endif
            else
        ...
        !if(icore==score) print *, 'Samarium prod.', samarium
        !if(icore==score) print *, 'Samarium abs.', samabs
        ...
        deallocate(mat%ogxs); allocate(mat%ogxs(1:num_iso,1:4)); mat%ogxs(1:num_iso,1:4) = 0.0d0
        i = 0 
        do mt_iso=1, mat%n_iso
            ...
            if (tmp /= 0 ) then 
                ...
                if (icore==score) then 
                    ...
                    ! ADENS
                    write(prt_bumat, '(a15,e14.5,e14.5)') ace(mat%ace_idx(mt_iso))%library, mat%numden(mt_iso)*barn, ace(mat%ace_idx(mt_iso))%atn*mat%numden(mt_iso)*mat%vol/N_AVOGADRO*m_n
                    ! A
    ...
    subroutine tally_burnup (imat, distance, wgt, erg)
        ...
        mat%flux = mat%flux + wgt*distance !Volume-integrated flux tally (not normalized)
         ...
         ! TEMPORARY... need to reduce
            ...
            micro_d = ace(iso)%sigd(ierg) + ipfac*(ace(iso)%sigd(ierg+1)-ace(iso)%sigd(ierg))
            !$OMP ATOMIC
            mat%ogxs(iso,1) = mat%ogxs(iso,1) + micro_d*wgt*distance*barn !(n,g)
            !$OMP ATOMIC
            mat%ogxs(iso,2) = mat%ogxs(iso,2) + getxs(16,iso,erg,ierg)*wgt*distance*barn !(n,2n)
            !FISSIONABLE
            if(ace(iso)%jxs(21)/=0 .or. allocated(ace(iso)%sigf)) then
                micro_f = ace(iso)%sigf(ierg) + ipfac*(ace(iso)%sigf(ierg+1)-ace(iso)%sigf(ierg))
                !$OMP ATOMIC
                mat%ogxs(iso,3) = mat%ogxs(iso,3) + getxs(17,iso,erg,ierg)*wgt*distance*barn !(n,3n)
                !$OMP ATOMIC
                mat%ogxs(iso,4) = mat%ogxs(iso,4) + micro_f*wgt*distance*barn !(n,f)
                fssn = ace_fssn(iso)
                if(fssn>0) then
                    !if(erg>fast_lb) then !Fast fission
                    !    !$OMP ATOMIC
                    !    fratio(fssn,3) = fratio(fssn,3) + micro_f*wgt*distance*barn
                    !elseif(erg<thermal_ub) then !Thermal fission
                    !    if(erg<0.0251d-6) then
                    !        !$OMP ATOMIC
                    !        fratio(fssn,1) = fratio(fssn,1) + micro_f*wgt*distance*barn
                    !    else
                    !        g = (erg-0.0251d-6)/(0.5d0-0.0251d-6)
                    !        !$OMP ATOMIC
                    !        fratio(fssn,1) = fratio(fssn,1) + micro_f*wgt*distance*barn*(1.d0-g)
                    !        !$OMP ATOMIC
                    !        fratio(fssn,2) = fratio(fssn,2) + micro_f*wgt*distance*barn*g
                    !    endif
                    !else !Epithermal
                    !    if(erg>0.5d0) then
                    !        !$OMP ATOMIC
                    !        fratio(fssn,2) = fratio(fssn,2) + micro_f*wgt*distance*barn
                    !    else
                    !        g = (erg-0.0251d-6)/(0.5d0-0.0251d-6)
                    !        !$OMP ATOMIC
                    !        fratio(fssn,1) = fratio(fssn,1) + micro_f*wgt*distance*barn*(1.d0-g)
                        ...
                        elseif(erg>=Ep(nE)) then !Solely fast
                            ...
                            do eg = 1,nE-1 !Already nE >= 2
                                if(erg>=Ep(eg) .and. erg<Ep(eg+1)) then
                                    ...
                                    !$OMP ATOMIC
                                    fratio(fssn,eg) = fratio(fssn,eg) + &
                                        micro_f*wgt*distance*barn*(1.d0-g)
                                    !$OMP ATOMIC
                                    fratio(fssn,eg+1) = fratio(fssn,eg+1) + &
                                        micro_f*wgt*distance*barn*g
                                endif
                            enddo
                        endif
                    endif
                ...
                !$OMP ATOMIC
                mat%ogxs(iso,3) = mat%ogxs(iso,3) + getxs(107,iso,erg,ierg)*wgt*distance*barn !(n,a)
                !$OMP ATOMIC
                mat%ogxs(iso,4) = mat%ogxs(iso,4) + getxs(103,iso,erg,ierg)*wgt*distance*barn !(n,p)
            endif
         enddo
    ...
    subroutine setmat
        ...
        do imat = 1, n_materials
            ...
            allocate(materials(imat)%ogxs(1:num_iso,1:4))
            materials(imat)%ogxs(:,:) = 0.0d0
    ...
    subroutine MPI_reduce_burnup 
        ...
        do imat = 1, n_materials
            ...
            allocate(rcvbufarrlong(1:num_iso*4))
            allocate(sndbufarrlong(1:num_iso*4))
            ...
            call MPI_ALLREDUCE(sndbufarrlong, rcvbufarrlong, num_iso*4, MPI_DOUBLE_PRECISION, MPI_SUM, core, ierr)                    
