...
subroutine simulate_history(bat,cyc)
    ...
    !if(.not.allocated(lambda_test)) allocate(lambda_test(npg))
    !lambda_test = 0.D0
    ...
    !call MPI_REDUCE(gen_prompt,rcv_buf,1,MPI_REAL8,MPI_SUM,score,MPI_COMM_WORLD,ierr)
    !gen_prompt = rcv_buf

    !call MPI_REDUCE(denom_prompt,rcv_buf,1,MPI_REAL8,MPI_SUM,score,MPI_COMM_WORLD, ierr)
    !denom_prompt= rcv_buf

    !call MPI_REDUCE(lambda_test,rcv_buf,1,MPI_REAL8,MPI_SUM,score,MPI_COMM_WORLD,ierr)
    !lambda_test = rcv_buf
    
	if (icore == score) avg_power = avg_power + cyc_power
    
    !if (icore == score) print *, 'LAMBDA1', lambda_test
    ...
    if (curr_cyc > n_inact+latent .and. icore == score) then
        do i = 1,8
    ...
    !if(do_fuel_mv) print *, 'LEAK', MSR_leak, '/', isize
...
! 2. tracking 하면서 새로운 dynamic_bank 채움
	...
	if (curr_cyc > n_pcqs_inact) then
        !print *, 'BETT', icore, PKE_tally(1:npg)
        !print *, 'LAMT', icore, PKE_tally(1+npg:2*npg)
        !print *, 'GENT', icore, PKE_tally(2*npg+2), PKE_tally(2*npg+1)
        !print *, 'ZZZT', icore, PKE_tally(2*npg+2)
		do i = 1, npg 
			if(PKE_tally(2*npg+1)/=0) PKE_beta_d(i) = PKE_beta_d(i) + PKE_tally(i) / PKE_tally(2*npg+1)
			if(PKE_tally(npg+i) /=0)  PKE_lambda(i) = PKE_lambda(i) + PKE_tally(i) / PKE_tally(npg+i)
		...
		enddo 
		if(PKE_tally(2*npg+1)/=0) PKE_gen = PKE_gen + PKE_tally(2*npg+2) / PKE_tally(2*npg+1)
...
subroutine bank_initialize(this)
	...
	subroutine draw_geometry() 
		...
		do i = 1, n_materials
			...
			enddo 
            if(materials(i)%mat_type == 4) mat_rgb(i,1:3) = 0.d0 !VOID
		enddo 
		rgb_blk(1:3) = 0
		
	    do i = 1,n_materials
            write(*,*) trim(materials(i)%mat_name), mat_rgb(i,1:3)
        enddo
