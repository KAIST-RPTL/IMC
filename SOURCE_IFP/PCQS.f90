module PCQS
	...
	subroutine solve_PKE() 
		...
		!> Calculate PKE parameters
		    ...
		    print *, 'BETA', PKE_beta_d
            print *, 'LAM ', PKE_lambda
            print *, 'GEN ', PKE_gen
            print *, 'KEFF', PKE_keff
	...
	subroutine collision_pcqs_MG(p) 
		...
		if (XS_MG(p%material)%sig_fis(p%g) > 0) then 
			...
			if (delayed) then 
                ...
                !ADJOINT
                if(latent>1) thread_bank(bank_idx)%delayedarr(1:latent-1) &
                    = thread_bank(bank_idx)%delayedarr(2:latent)
                thread_bank(bank_idx)%delayedarr(latent) = i_group
                if(latent>1) thread_bank(bank_idx)%nlifearr(1:latent-1) &
                    = thread_bank(bank_idx)%nlifearr(2:latent)
                thread_bank(bank_idx)%nlifearr(latent) = p%trvltime
                if(latent>1) thread_bank(bank_idx)%delayedlam(1:latent-1) &
                    = thread_bank(bank_idx)%delayedlam(2:latent)
                thread_bank(bank_idx)%delayedlam(latent) = MGD(p%material)%lambda(i_group)
                ...
                if(latent>1) thread_bank(bank_idx)%delayedlam(1:latent-1) &
                    = thread_bank(bank_idx)%delayedlam(2:latent)
                thread_bank(bank_idx)%delayedlam(latent) = 0.d0
			endif 
    ...
    enddo
		...
		if (micro_xs(4)>0) then 
			...
			if (delayed) then 
                ...
                if(latent>1) thread_bank(bank_idx)%delayedlam(1:latent-1) &
                    = thread_bank(bank_idx)%delayedlam(2:latent)
                thread_bank(bank_idx)%delayedlam(latent) = ace(iso) % prcr(i_group) % decay_const
...
subroutine collision_PCQS_CE_init (p)
	...
	subroutine set_PCQS_source() 
        ...
        !print *, 'COMB', size(temp_bank), size(fission_bank), isize
