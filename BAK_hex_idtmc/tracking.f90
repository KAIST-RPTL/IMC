...
!    endif 
	...
	!if (p%coord(1)%xyz(1) /= 0.0) then
	!    x = p%coord(1)%xyz(1)
	!	y = p%coord(1)%xyz(2)
	!    if (abs(x+y) > abs(x-y)) maindiag = maindiag + 1
	!	if (abs(x+y) < abs(x-y)) contradiag = contradiag + 1
	!	print *, maindiag, contradiag
	!end if
...
!    endif
    ...
    !print '(A,<8>F10.4)', 'POS', p%coord(1)%xyz, p%coord(1)%uvw, d_collision, d_mesh
...
!        p%tet_face = 0
        ...
        !print *, 'FLAG1', curr_cyc, cells(7) % pos_surf_idx(1)
        if ( fmfdon .and. inside_mesh ) then
...
!        end if
        !print *, 'FLAG1.2', curr_cyc, cells(7) % pos_surf_idx(1)

        if (E_mode == 0) then 
            call collision_MG(p)
            !print *, 'FLAG1.3', curr_cyc, cells(7) % pos_surf_idx(1)
        else !(E_mode == 1) 
            call collision_CE(p)
        endif
