module COSAMPLING
    ...
    !use DEPLETION_MODULE, only: istep_burnup
    implicit none
...
!    end function
    ...
    function erfmat(M) result(RES)
    ! Returns ERF applied mat (nxn)
    implicit none
    real(8), intent(in) :: M(:,:)
    real(8) :: RES(3,3)
    integer :: i,j
    ...
    function ierfmat(M) result(RES)
    ! Returns IERF applied mat (nxn)
    implicit none
    include 'mkl_vml.f90'
    real(8), intent(in) :: M(:,:)
    real(8) :: inp(1), tmp(1)
    ...
    do i = 1, 3
        do j = 1, 3
            if(i==j .or. abs(M(i,j))>=1d0) then
                tmp(1) = 0d0
            else
                inp = M(i,j)
                call vderfinv(1, inp, tmp)
            endif
            RES(i,j) = tmp(1)
        enddo
    enddo
