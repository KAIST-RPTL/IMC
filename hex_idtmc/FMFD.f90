module FMFD
	...
	use hex_variables
	use hex_fmfd, only: hf_fmfd_calc, hf_process ! LINKPOINT
...
subroutine FMFD_initialize_thread() 
    ...
    if ( .not. allocated(fm_thread) ) then
        if(dduct < 0.0) then
            allocate(fm_thread(nfm(1),nfm(2),nfm(3)))
        else
            !print *, 'SIZE', x_max, y_max, z_max
            allocate(fm_thread(x_max, y_max, z_max))
        endif
    endif
    ...
    if ( DTMCBU ) then
        ...
        if ( .not. allocated(thflux) ) then
            if(dduct < 0.0) then
                ...
                allocate(thflux(x_max, y_max, z_max,nrings))
            endif
        end if
...
subroutine BASE_FMFD_CALCULATION(bat,cyc,phi1)
	...
	if (dduct > 0.0) then ! LINKPOINT
	    call hf_fmfd_calc(bat, cyc, phi1)
		return
	end if
