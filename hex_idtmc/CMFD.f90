...
subroutine FM_TRANSPOSE
    ...
    if ( .not. allocated(Mfm1) ) then
        allocate(Mfm1(nfm(1)*nfm(2)*nfm(3),7), stat = err_val)
		if (err_val /= 0) print *, "ALLOCATE FAIL - FM_TRANSPOSE MFML"
        allocate(imn(nfm(1),nfm(2),nfm(3)), stat = err_val)
		if (err_val /= 0) print *, "ALLOCATE FAIL - FM_TRANSPOSE IMN"
...
subroutine CM_TRANSPOSE
    ...
    if ( .not. allocated(Mcm1) ) then
        allocate(Mcm1(ncm(1)*ncm(2)*ncm(3),7), stat = err_val)
		if (err_val /= 0) print *, "ALLOCATE FAIL - CM_TRANSPOSE MCML"
        allocate(imn(ncm(1),ncm(2),ncm(3)), stat = err_val)
		if (err_val /= 0) print *, "ALLOCATE FAIL - CM_TRANSPOSE IMN"
...
subroutine L_MATRIX(Dt,Dh,abso)
    ...
    ! -------------------------------------------------------------------------
    !   source term
    allocate(deno(nfm(1),nfm(2),nfm(3)), stat = err_val)
	if (err_val /= 0) print *, "ALLOCATE FAIL - CMFD DENO"
    do ii = 1, ncm(1); id0(1) = (ii-1)*fcr
    do jj = 1, ncm(2); id0(2) = (jj-1)*fcr
    do kk = 1, ncm(3); id0(3) = (kk-1)*fcz
        if ( OUT_OF_ZZ(ii,jj) ) cycle
