...
subroutine simulate_history(bat,cyc)
	...
	! --- ORDER THE BANK
	DO i = 1,isize
		source_bank(i)%i_Current = i
	END DO
	! (THIS INCURS COMPUTING BUREN)	! --- STORE THE BANK FOR IFP ASSESSMENT (SHIFTING mat_source_bank INCURS BURDEN)
	! (THIS INCURS COMPUTING BUREN)	IF(cyc > 10) THEN
	! (THIS INCURS COMPUTING BUREN)		IF(NOT(ALLOCATED(mat_source_bank))) THEN
	! (THIS INCURS COMPUTING BUREN)			ALLOCATE(mat_source_bank(FLOOR(REAL(isize,8)*1.1d0 + TINY_BIT),latent))
	! (THIS INCURS COMPUTING BUREN)		END IF
	! (THIS INCURS COMPUTING BUREN)		DO i = 1,latent-1
	! (THIS INCURS COMPUTING BUREN)			mat_source_bank(:,i) = mat_source_bank(:,i+1)
	! (THIS INCURS COMPUTING BUREN)		END DO
	! (THIS INCURS COMPUTING BUREN)		mat_source_bank(1:isize,latent) = source_bank(:)
	! (THIS INCURS COMPUTING BUREN)	END IF
	
	IF(cyc > n_inact) THEN
		IF(NOT(ALLOCATED(mat_source_bank))) THEN
			ALLOCATE(mat_source_bank(FLOOR(REAL(isize,8)*1.1d0 + TINY_BIT),latent))
		END IF
		AHRI = MOD((cyc-n_inact),latent)
		IF(AHRI == 0) AHRI = latent
		mat_source_bank(1:isize,AHRI) = source_bank(:)
	END IF	
