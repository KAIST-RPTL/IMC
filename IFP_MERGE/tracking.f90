...
!    if ( th_on ) call TH_DISTANCE(p,j_xyz,d_TH)
    ...
    if(curr_cyc > n_inact .and. materials(p%material)%fissionable) then 
		...
		! TSOH-IFP: do_IFP ALWAYS TRUE
		
        ! (TSOH-IFP)	!$omp atomic
        ! (TSOH-IFP)	denom= denom + distance*p%wgt*macro_xs(4)
        ! (TSOH-IFP)	!$omp atomic
        ! (TSOH-IFP)	gen_numer = gen_numer + distance*p%wgt*p%nlifearr(1)*macro_xs(4)
        ! (TSOH-IFP)	if(p%delayedarr(1)>0) then
        ! (TSOH-IFP)	    !$omp atomic
        ! (TSOH-IFP)	    beta_numer(p%delayedarr(1)) = beta_numer(p%delayedarr(1)) + distance*p%wgt*macro_xs(4)
        ! (TSOH-IFP)	    !$omp atomic
        ! (TSOH-IFP)	    lam_denom(p%delayedarr(1))  = lam_denom(p%delayedarr(1)) + distance*p%wgt*macro_xs(4)/p%delayedlam(1)
        ! (TSOH-IFP)	endif

		! +++ FIND THE ULTIMATE PARENT (애비가 누구야?)
		IF(curr_cyc > n_inact + latent) THEN
			i_Parent = p%i_Parent
			AHRI = MOD((curr_cyc-n_inact),latent)
			IF(AHRI == 0) AHRI = latent
			DO i = 1,latent-1
				SONA = AHRI-i+1
				IF(SONA <= 0) THEN
					SONA = SONA + latent
				END IF
				i_Parent = mat_source_bank(i_Parent,SONA)%i_Parent
			END DO
			AHRI = MOD((curr_cyc-n_inact),latent) + 1
			!$OMP ATOMIC
			denom = denom + distance*p%wgt*macro_xs(4)
			!$OMP ATOMIC
			gen_numer = gen_numer + distance*p%wgt*macro_xs(4) * mat_source_bank(i_Parent,AHRI)%nlife
			if(mat_source_bank(i_Parent,AHRI)%dIMT > 0) then
				!$OMP ATOMIC
				beta_numer(mat_source_bank(i_Parent,AHRI)%dIMT) = beta_numer(mat_source_bank(i_Parent,AHRI)%dIMT) + distance*p%wgt*macro_xs(4)
				!$OMP ATOMIC
				lam_denom (mat_source_bank(i_Parent,AHRI)%dIMT) = lam_denom (mat_source_bank(i_Parent,AHRI)%dIMT) + distance*p%wgt*macro_xs(4)/mat_source_bank(i_Parent,AHRI)%dlam
			endif
		END IF
    endif 
